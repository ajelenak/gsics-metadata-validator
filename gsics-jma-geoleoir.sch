...
<!--
  This schema validates GSICS GEO-LEO-IR netCDF files produced by JMA.
  The rules here represent the minimal valid metadata content that every
  GEO-LEO-IR file produced by JMA must provide.
  
  The schema depends on the gsics-geoleoir-core.sch file. Inclusion of this
  file is done using a novel feature that may not be supported yet by various
  XML editors. The best approach is to use the XSLT scripts from 
  http://www.schematron.com/implementation.html and run the iso_dsdl_include.xsl
  script on this file. The resulting Schematron file can then be opened in an
  XML editor for testing.
-->
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <pattern is-a="global-attribute" id="standard_name_vocabulary">
    ...
    <param name="attrValue" value="'CF Standard Name Table (Version 21, 12 January 2013)'"/>
    <param name="attrType" value="''"/>
  </pattern>
  <pattern is-a="global-attribute" id="naming_authority">
    <param name="attrName" value="'naming_authority'"/>
    <param name="attrValue" value="'jp.go.jma.msc.gsics'"/>
    <param name="attrType" value="''"/>
  </pattern>
  <pattern is-a="global-attribute" id="creator_name">
    <param name="attrName" value="'creator_name'"/>
    <param name="attrValue" value="'System Engineering Division, Meteorological Satellite Center, Japan Meteorological Agency'"/>
    <param name="attrType" value="''"/>
  </pattern>
  <pattern is-a="global-attribute" id="creator_email">
    <param name="attrName" value="'creator_email'"/>
    <param name="attrValue" value="'calmstr@dpc.kishou.go.jp'"/>
    <param name="attrType" value="''"/>
  </pattern>
  <pattern is-a="global-attribute" id="creator_url">
    <param name="attrName" value="'creator_url'"/>
    <param name="attrValue" value="'http://mscweb.kishou.go.jp/monitoring/calibration.htm'"/>
    <param name="attrType" value="''"/>
  </pattern>
  ...
  <pattern id="monitored_instrument">
    ...
    <rule context="/nc:netcdf/nc:attribute[@name = 'monitored_instrument']">
      <assert test="@value = ('MTSAT2+Imager')">Global attribute "<value-of select="@name"/>": Invalid value.</assert>
    </rule>
  </pattern>
  <pattern id="reference_instrument">
    ...
    <rule context="/nc:netcdf/nc:attribute[@name = 'reference_instrument']">
      <assert test="@value = ('MetOpA+IASI')">Global attribute "<value-of select="@name"/>": Invalid value.</assert>
    </rule>
  </pattern>
  ...
  <pattern is-a="variable" id="slope">
    ...
    <param name="varUnits" value="'(mW m-2 sr-1(cm-1)-1)-1'"/>
